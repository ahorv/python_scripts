...
uses smbus instead adafruit's i2c library
...
class TCS34725(object):
    ...
    def __init__(self, integration_time=TCS34725_INTEGRATIONTIME_2_4MS,
                 gain=TCS34725_GAIN_4X, address=TCS34725_ADDRESS, bus_num=1):
        """Initialize the TCS34725 sensor."""
        # Setup smbus interface for the device.
    ...
    def _readU8(self, reg):
        """Read an unsigned 8-bit register - Register addresses must be OR'ed with 0x80"""
        # return self._device.readU8(self.TCS34725_COMMAND_BIT | reg)
        return self._device.read_byte_data(self.TCS34725_ADDRESS, self.TCS34725_COMMAND_BIT |reg)

    def _readU16LE(self, reg):
        """Read a 16-bit little endian register."""
        # return self._device.readU16LE(self.TCS34725_COMMAND_BIT | reg)
        return self._device.read_word_data(self.TCS34725_ADDRESS, self.TCS34725_COMMAND_BIT | reg)

    def _write8(self, reg, value):
        """Write a 8-bit value to a register."""
        # self._device.write8(TCS34725_COMMAND_BIT | reg, value)
        self._device.write_byte_data(self.TCS34725_ADDRESS, self.TCS34725_COMMAND_BIT | reg, value)
    ...
    def enable(self):
        ...
        time.sleep(0.01)
        self._write8(self.TCS34725_ENABLE, (self.TCS34725_ENABLE_PON | self.TCS34725_ENABLE_AEN))

    def disable(self):
        """Disable the chip (power down)."""
        # Flip off the power on and enable bits.
        reg = self._readU8(self.TCS34725_ENABLE)
        reg &= ~(self.TCS34725_ENABLE_PON | self.TCS34725_ENABLE_AEN)
        self._write8(self.TCS34725_ENABLE, reg)
    ...
    def get_integration_time(self):
        ...
        """
        return self._readU8(self.TCS34725_ATIME)
    ...
    def get_gain(self):
        ...
        """
        return self._readU8(self.TCS34725_CONTROL)
    ...
    def get_raw_data(self):
        ...
        r = self._readU16LE(self.TCS34725_RDATAL)
        g = self._readU16LE(self.TCS34725_GDATAL)
        b = self._readU16LE(self.TCS34725_BDATAL)
        c = self._readU16LE(self.TCS34725_CDATAL)
    ...
    def set_interrupt(self, enabled):
        ...
        enable_reg = self._readU8(self.TCS34725_ENABLE)
        if enabled:
            enable_reg |= self.TCS34725_ENABLE_AIEN
        else:
            enable_reg &= ~self.TCS34725_ENABLE_AIEN
        self._write8(self.TCS34725_ENABLE, enable_reg)
        time.sleep(1)
...
if __name__ == '__main__':
    ...
    try:
        rgb_sensor = TCS34725()
        rgb_sensor.set_interrupt(True)

        r, g, b, c = rgb_sensor.get_raw_data()
        print('color: red={} green={} blue={} clear={}'.format(r, g, b, c))
        lux = rgb_sensor.calculate_lux(r,g,b)
